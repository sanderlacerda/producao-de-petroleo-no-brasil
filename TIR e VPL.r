...
par(ask=TRUE)
par(mfrow=c(1,2))
mostra <- data.frame(precos_pet,tx_int_ret,v_p_l)

# grava resultados
write.table(mostra,file = "c:/simulacao/tir_vpl.txt")

################################
# TIR contra PRE?O DO PETR?LEO #
################################

plot(
	precos_pet,tx_int_ret,
	type=c("l"),pch=1,bty="l",lwd=c(2),
	col=c('royalblue1'),lty = c(1),
	xlab="Pre?o do petr?leo (US$ por barril)",ylab="TIR (%)",main="Taxa interna de retorno",cex.main=1)
grid(NULL,NULL,lwd = 1,col='gray41',lty="dotted")

#################################
#  VPL contra PRE?O DO PETR?LEO #
#################################

grafico <- data.frame(v_p_l/1000000000,seq(0,0,length.out=length(precos_pet)))
matplot(
	precos_pet,grafico,
	type=c("l","l"),pch=c(1,1),bty="l",lwd=c(2,2),
	col=c('royalblue1','black'),lty = c(1,1),ylim=c(-50,300),xaxs="i",
	xlab="Pre?o do petr?leo (US$ por barril)",ylab="VPL (US$ bilh?es)",main="Valor presente l?quido",cex.main=1)
grid(NULL,NULL,lwd = 1,col='gray41',lty="dotted")

savePlot("TIR e VPL",type=c("jpeg"))
